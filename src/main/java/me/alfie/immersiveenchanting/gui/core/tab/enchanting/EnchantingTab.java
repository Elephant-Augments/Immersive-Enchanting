...
public class EnchantingTab {
    ...
    public void checkToolSlotUpdated() {
        ...
        if (!ItemStack.isSameItemSameTags(item, lastStack)) {
            lastStack = item.copy();
            onToolSlotUpdate();
        }
    }
    ...
    public void onNodeHeld() {
        ...
        if(heldNode instanceof EnchantingNode enchantingNode && heldTime > HOLD_THRESHOLD && ServerConfig.isEnchantmentRemovalAllowed()) {
            ...
            int highestUnlockedLevel = screen.getMenu().getToolSlotItem().getEnchantmentLevel(enchantingNode.getEnchantmentHolder().value());

            if (nodeEnchantmentLevel == highestUnlockedLevel) {
                heldNode = null;
                ModPackets.INSTANCE.sendToServer(new RemoveEnchantmentPacket(
                        enchantingNode.getEnchantmentHolder().unwrapKey().get(),
