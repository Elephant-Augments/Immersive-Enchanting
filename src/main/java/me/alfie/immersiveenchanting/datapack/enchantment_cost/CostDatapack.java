...
public class CostDatapack extends ModDatapack<CostData, CostRegistry> {
     ...
     */
    private static ResourceId remapIdentifierPath(ResourceLocation originalId) {
    ...
    public static void resolveClientRegistry(ClientDatapackUpdatedEvent event) {
        CostRegistry.client().resolveEnchantmentHolders(event.getPlayer().level().registryAccess());
    ...
    private static void sendWarningMessages(Player player) {
        ...
        List<Holder<Enchantment>> allEnchantments = EnchantmentUtil.getAllEnchantmentsInRegistry(player.level().registryAccess());
        ...
        for (Holder<Enchantment> enchantmentHolder : CostRegistry.server().getAllEnchantmentHolders()) {
            ...
            for (int level = 0; level < levelCosts.maxLevel(); level++) {
                if (levelCosts.getLevel(level+1).costs().isEmpty()) {
                    brokenIds.add(enchantmentHolder.unwrapKey().get().location().toString());
                }
            }
        }
